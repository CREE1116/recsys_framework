...
# exp05: Spectral Bias Analysis (Singular Direction별 인기 편향 집중도 분석)
...
# exp09: Alpha Transferability   (최적 α의 데이터셋 간 이식성 및 성능 보존 검증)
...
from aspire_experiments.exp05_spectral_bias     import run_spectral_bias
...
from aspire_experiments.exp09_alpha_transferability import run_alpha_transfer
...
def main():
    ...
    for dataset in args.datasets:
        ...
        # 05. Spectral Bias
        if "exp05" not in args.skip:
            print("\n[Exp 05] Spectral Bias Analysis")
            run_spectral_bias(dataset, target_energy=args.energy)
        ...
        # 09. Alpha Transferability
        # Note: This experiment logic transfers alpha FROM the current dataset TO other datasets.
        # For a clean summary run, it is most often run with ml100k -> others. 
        if "exp09" not in args.skip and dataset == "ml100k":
            # For automation, if running ml100k, we check transfer to ml1m and steam.
            targets = [ds for ds in args.datasets if ds != "ml100k"]
            if targets:
                print(f"\n[Exp 09] Alpha Transferability (Source: {dataset})")
                run_alpha_transfer(dataset, targets, energy=args.energy, n_trials=args.trials)
        ...
        try:
            ...
            # Currently ignoring r8, r9 because their aggregate involves more complex multi-dataset extraction
            # and is natively saved as CSV in their respective folders.
