...
class ASPIRE_Median(BaseModel):
    """
    ASPIRE_Median: Robust Median Slope Equilibrium Engine
    [Robust Logic]
    - Instead of Hill Estimator (Mean), it use Median of local slopes.
    - Highly stable against outliers in spectral distribution.
    - Prevents divergence caused by extreme values in the head or tail.
    """
    def __init__(self, config, data_loader):
        super(ASPIRE_Median, self).__init__(config, data_loader)
    ...
    def _infer_gamma_slope_consistency(self, lambda_obs, anchor_ext):
        ...
        for i in range(self.max_iter):
            ...
            # 2. Measure Slope using MEDIAN of Local Log-Log Diff
            log_s = np.log(s_k + eps)
            log_r = np.log(np.arange(1, len(s_k) + 1))
            
            # local_slopes = -d(log_s) / d(log_r)
            local_slopes = -np.diff(log_s) / (np.diff(log_r) + eps)
            b = np.median(local_slopes) # Robust estimator
            
            # 3. Direct Update Rule
            gamma_L = 1.0 / (1.0 + b + eps)
            gamma_L = np.clip(gamma_L, 0.01, 1.0)
            
            self._log(f"[ASPIRE-Median] Iter {i+1:2d} | \u03b3_L: {prev_gamma_L:.4f} | Median b: {b:.4f}")
            
            final_b = b
            if abs(gamma_L - prev_gamma_L) < self.tol:
                self._log(f"[ASPIRE-Median] Engine Converged at Iter {i+1}")
                break
    ...
    def _build(self, X_sparse, dataset_name):
        ...
        print(f"[ASPIRE-Median] Accelerated Build: \u03b3_L={self.gamma_L:.4f} | W: {W_np.shape}")
    ...
    def _save_spectral_analysis(self, lambda_obs, tau, h, anchor_ext):
        ...
        axes.loglog(ranks, tau/l0, label=rf'Signal ($\gamma_L={self.gamma_L:.4f}$)', color='#8e44ad', linewidth=2)
