...
from src.models.csar.LIRALayer import LightPowerLIRALayer
from src.utils.gpu_accel import SVDCacheManager

class LightPowerLIRA(BaseModel):
    """
    LightPowerLIRA - Scalable LIRA with Power Sharpening and Thresholding.
    """
    def __init__(self, config, data_loader):
        super(LightPowerLIRA, self).__init__(config, data_loader)
        ...
        # LightPowerLIRA Layer
        self.lira_layer = LightPowerLIRALayer( 
            k=self.k,
        ...
        # Initialize
        print(f"[LightPowerLIRA] Initialized with k={self.k}, λ={self.reg_lambda}, power={self.power}, threshold={self.threshold}, Visualize={self.visualize}")
        
        # Build Sparse Matrix from DataLoader
        self.train_matrix_csr = self._build_sparse_matrix(data_loader)
        
        self.lira_layer.build(self.train_matrix_csr, dataset_name=config.get('dataset_name'))
        self.lira_layer.to(self.device)
    ...
    def fit(self, data_loader):
        print(f"\n[LightPowerLIRA] Training (k={self.k}, λ={self.reg_lambda}, Power={self.power}, Threshold={self.threshold})")
        print("="*60)
        ...
        if self.visualize:
            print(f"[LightPowerLIRA] Saving visualizations to {analysis_dir}...")
