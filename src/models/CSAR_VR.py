...
class CSAR_VR(BaseModel):
    """
    CSAR_V (Variational Interest) + BPR Loss
    """
    def __init__(self, config, data_loader):
        super(CSAR_VR, self).__init__(config, data_loader)
    ...
    def forward(self, users):
        ...
        residual_scores = torch.einsum('bk,nk->bn', user_embs, all_item_embs)
        return interest_scores + residual_scores
    ...
    def predict_for_pairs(self, user_ids, item_ids):
        ...
        # 2. Residual Connection
        residual_scores = torch.einsum('bk,nk->bn', user_embs, item_embs)
        return interest_scores + residual_scores
