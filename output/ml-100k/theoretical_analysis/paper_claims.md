...
> "수학적으로 **Tail 추천 비중을 10% 늘리면, Hit Rate는 6.0% 감소하는 것이 
> 이론적 정상치(Expected Baseline)**이다. 그러나 우리 모델(infinity_ae__BEST_infinity_ae_InfinityAE_Lambda_Search)은 
> LongTailCoverage가 0.0%에 달하면서도 
> NDCG 0.0651를 유지하여, 단순 확률 이동 대비 높은 효율성을 입증한다."
...
> **infinity_ae__BEST_infinity_ae_InfinityAE_Lambda_Search의 PEI는 inf**이다. 
> 이는 정확도-다양성 상충 관계에서 우리 모델이 
> **기대치보다 inf% 더 효율적인 파레토 프론티어(Pareto Frontier)에 위치**함을 보여준다."
